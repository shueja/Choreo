...
#include "segment.hpp"
namespace choreo {
/// @brief Converts the waypoints and constraints in the Parameters into a
/// vector of Segments, where each segment contains the constraints that apply
/// to it. This is done by first validating and processing the constraints to
/// determine which waypoints they apply to, then creating segments for each
/// waypoint and assigning the appropriate constraints to each segment. If a
/// Segment's start Waypoint is marked as an initial guess (i.e. it is not the
/// start or end of the path, is not referenced by any constraints, and does not
/// have its translation or heading fixed), then it will be marked to coalesce
/// with the previous segment, which means that it will be treated as part of
/// the previous segment in the optimization problem instead of as a separate
/// segment with its own constraints. This allows for waypoints that are only
/// meant to provide an initial guess for the optimizer to be included in the
/// path without forcing them to be treated as hard constraints.
/// @param params The Parameters containing the waypoints and constraints to
/// convert into segments.
/// @return A vector of Segments, where each Segment contains a start Waypoint
/// and the constraints that apply to it.
...
std::vector<Segment> convert_to_segments(const Parameters& params) {
  ...
  if (params.waypoints.size() < 2) {
    throw std::runtime_error(
        "Trajectory must have at least two waypoints; cannot generate "
        "segments");
  }

  std::vector<ConstraintIDX> constraints;
  // First, we need to convert the constraints from the parameters into a more
  // convenient format for processing. We also need to validate that the 'from'
  // and 'to' indices are within bounds of the waypoints vector, and print
  // warnings for any malformed constraints. This filters out constraints with
  // invalid indices.
  ...
  // Clean up malformed data: if 'to' is the same as 'from', we can treat it as
  // a waypoint constraint instead of a segment constraint, since it only
  // applies to one waypoint and doesn't actually constrain the segment between
  // two waypoints. If 'to' is less than 'from', we can swap them so that 'from'
  // is always less than 'to', which simplifies processing later on.
  ...
  // Convert the constraint data into a format independent of the locations of
  // its from and to waypoints. Later, the constraints will be split to each
  ...
  // guesses.
  // SAFETY: The constraints have already been validated to ensure that their
  // 'from' and 'to' indices are within bounds of the waypoints vector, so these
  // accesses are safe.
  ...
  // we already verified that to, if it exists, is greater than from, so we can
  // just apply the constraint to every segment between from and to, exclusive
  // of to.
  for (const auto& constraint : constraints) {
...
wpi::units::second_t Segment::estimate_segment_time(
    const Segment& next, const RobotConfig& config) const {
