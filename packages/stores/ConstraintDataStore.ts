...
import { Expr, isExpr } from "@choreo/document/math/Expr";
import {
  ConstraintData,
 ...
 * boolean for T extends boolean,
 * and errors if T is neither
 */
...
>(def: ConstraintDefinition<K>): ConstraintDataStore<K> {
  ...
  const store = types
    ...
    .views((self) => ({
      ...
      get def() : ConstraintDefinition<K> {
        return ConstraintDefinitions[def.type];
      }
    }))
    .actions((self) => ({
      deserialize(ser: D) {
