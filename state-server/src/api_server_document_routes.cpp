...
void ApiServer::RegisterDocumentRoutes() {
  ...
  const auto record_scope_mutation = [this](std::string_view scope_key,
  ...
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
                                     const RouteParams&) {
        const auto project_scope = ProjectScopeKey();
        const auto before = CaptureScopeSnapshot(project_scope)
                                .value_or(wpi::util::json(nullptr));
  ...
  // Response: 200 with updated ProjectFile JSON and a fresh ETag.
  m_router.Register(
      HttpMethod::kPatch, "/api/v1/project",
      [this, &record_scope_mutation](const Request& request,
                                     const RouteParams&) {
        const auto project_scope = ProjectScopeKey();
        const auto before = CaptureScopeSnapshot(project_scope)
  ...
  m_router.Register(HttpMethod::kPost, "/api/v1/project/undo",
                    [this, &record_scope_mutation](const Request& request,
                      ...
                      const auto project_scope = ProjectScopeKey();
                      const auto current_revision = ProjectRevisionToken();
                      if (auto error = ValidateIfMatchPrecondition(
  ...
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          const auto before = CaptureScopeSnapshot(trajectory_scope)
                                  .value_or(wpi::util::json(nullptr));
          m_trajectories.emplace(uuid, std::move(created));
          ++m_trajectory_revisions[uuid];
          record_scope_mutation(trajectory_scope, "create_trajectory", before);
  ...
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request&,
  ...
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        const auto& trajectory_uuid_value = trajectory_uuid->get();
        const auto trajectory_scope = TrajectoryScopeKey(trajectory_uuid_value);
        const auto before = CaptureScopeSnapshot(trajectory_scope)
                                .value_or(wpi::util::json(nullptr));
  ...
  // Response: 200 with updated TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      HttpMethod::kPatch, "/api/v1/trajectories/{uuid}",
      [this, &record_scope_mutation](const Request& request,
                                     const RouteParams& params) {
        const auto trajectory_uuid = FindRouteParam(params, "uuid");
        if (!trajectory_uuid.has_value()) {
  ...
  // Preconditions: trajectory must exist, If-Match must match, and name must be
  // unique and non-empty. Body: { name }. Response: 200 with updated
  // TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          if (!body.is_object() || !body.contains("name") ||
            ...
            return InvalidJson("Request body must include string field 'name'");
          }
  ...
  // Preconditions: trajectory must exist, If-Match must match, and waypoint
  // UUID must be unique. Body: { waypoint, insertIndex? }. Response: 201 with
  // updated TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          return JsonModelResponseWithEtag(
              201, TrajectoryRevisionToken(it->second), traj_it->second);
        } catch (const std::exception& ex) {
          return choreo::rest_router::MakeJsonErrorResponse(400, "invalid_json",
                                                            ex.what());
        }
      });

  // Route: Patch a single waypoint.
  // Preconditions: trajectory and waypoint must exist, and If-Match must match
  // the trajectory ETag. Body: object-merge JSON or RFC6902 patch array over
  // Waypoint fields. Response: 200 with updated TrajectoryFile JSON and a fresh
  // ETag.
  m_router.Register(
      HttpMethod::kPatch,
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        const auto trajectory_scope = TrajectoryScopeKey(traj_param->second);
        const auto before = CaptureScopeSnapshot(trajectory_scope)
                                .value_or(wpi::util::json(nullptr));

        auto waypoint_index = FindByUuid(traj_it->second.params.waypoints,
  ...
  // Preconditions: trajectory and waypoint must exist, If-Match must match, and
  // at least one waypoint must remain. Body: none. Response: 204 with no body
  // after related references are cleaned up.
  m_router.Register(
      HttpMethod::kDelete,
      "/api/v1/trajectories/{uuid}/waypoints/{waypointUuid}",
      [this, &record_scope_mutation](const Request& request,
        ...
        constraints.erase(
            std::remove_if(constraints.begin(), constraints.end(),
                           [&waypoint_param](const Constraint& constraint) {
                             const auto matches_id = [&waypoint_param](
                                                         const WaypointID& id) {
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          for (const auto& uuid : order) {
            ...
            if (!index.has_value()) {
              return choreo::rest_router::MakeJsonErrorResponse(
                  400, "invalid_order", "order contains unknown waypoint UUID");
            }
          ...
          ++m_trajectory_revisions[traj_param->second];
          record_scope_mutation(trajectory_scope, "reorder_waypoints", before);
  ...
  // Preconditions: trajectory must exist, If-Match must match, and constraint
  // UUID must be unique. Body: { constraint, insertIndex? }. Response: 201 with
  // updated TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          if (FindByUuid(traj_it->second.params.constraints, constraint.uuid)) {
            return choreo::rest_router::MakeJsonErrorResponse(
                409, "uuid_conflict", "Constraint UUID already exists");
          }

          size_t insert_index = traj_it->second.params.constraints.size();
          std::string parse_error;
          if (!ParseInsertIndex(body, traj_it->second.params.constraints.size(),
                                insert_index, parse_error)) {
            return choreo::rest_router::MakeJsonErrorResponse(
                400, "invalid_json", parse_error);
          }
  ...
  // Preconditions: trajectory and constraint must exist, and If-Match must
  // match the trajectory ETag. Body: object-merge JSON or RFC6902 patch array
  // over Constraint fields. Response: 200 with updated TrajectoryFile JSON and
  // a fresh ETag.
  m_router.Register(
      HttpMethod::kPatch,
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        if (traj_param == params.end() || constraint_param == params.end()) {
          return BadRoute("Missing route parameter");
        }
        ...
        auto constraint_index = FindByUuid(traj_it->second.params.constraints,
        ...
        try {
          ...
          traj_it->second.params.constraints[*constraint_index] =
              std::move(updated);
          ++m_trajectory_revisions[traj_param->second];
          record_scope_mutation(trajectory_scope, "patch_constraint", before);
  ...
  // Preconditions: trajectory and constraint must exist, and If-Match must
  // match the trajectory ETag. Body: none. Response: 204 with no body.
  m_router.Register(
      HttpMethod::kDelete,
      "/api/v1/trajectories/{uuid}/constraints/{constraintUuid}",
      [this, &record_scope_mutation](const Request& request,
  ...
  // Preconditions: trajectory must exist and If-Match must match its ETag.
  // Body: { order: [constraintUuid, ...] } including every constraint exactly
  // once. Response: 200 with updated TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          ++m_trajectory_revisions[traj_param->second];
          record_scope_mutation(trajectory_scope, "reorder_constraints",
                                before);
          return JsonModelResponseWithEtag(
              200, TrajectoryRevisionToken(traj_param->second),
              traj_it->second);
  ...
  // Preconditions: trajectory must exist, If-Match must match, and marker UUID
  // must be unique. Body: { marker, insertIndex? }. Response: 201 with updated
  // TrajectoryFile JSON and a fresh ETag.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          markers.insert(markers.begin() + insert_index, std::move(marker));
          ++m_trajectory_revisions[traj_param->second];
  ...
  // Preconditions: trajectory and marker must exist, and If-Match must match
  // the trajectory ETag. Body: object-merge JSON or RFC6902 patch array over
  // EventMarker fields. Response: 200 with updated TrajectoryFile JSON and a
  // fresh ETag.
  m_router.Register(
      HttpMethod::kPatch, "/api/v1/trajectories/{uuid}/markers/{markerUuid}",
      [this, &record_scope_mutation](const Request& request,
                                     const RouteParams& params) {
        const auto traj_param = params.find("uuid");
        ...
        try {
          auto patch =
              wpi::util::json::parse_or_throw(std::string_view{request.body});
          auto merged = wpi::util::json(traj_it->second.events[*marker_index]);
          std::string patch_error;
          if (!ApplyObjectJsonPatch(merged, patch, patch_error)) {
  ...
  // Preconditions: trajectory and marker must exist, and If-Match must match
  // the trajectory ETag. Body: none. Response: 204 with no body.
  m_router.Register(
  ...
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          ++m_trajectory_revisions[traj_param->second];
          record_scope_mutation(trajectory_scope, "reorder_markers", before);
          return JsonModelResponseWithEtag(
              200, TrajectoryRevisionToken(traj_param->second),
              traj_it->second);
        } catch (const std::exception& ex) {
  ...
  // Preconditions: body mode must be merge or replace and bundle must contain
  // project plus trajectories. Body: { mode, bundle }. Response: 202 with
  // completed operation summary for creates, updates, and deletes.
  m_router.Register(
      ...
      [this, &record_scope_mutation](const Request& request,
        ...
        try {
          ...
          if (!body.is_object() || !body.contains("mode") ||
              !body.at("mode").is_string() || !body.contains("bundle") ||
          ...
          if (!bundle.contains("project") || !bundle.contains("trajectories") ||
          ...
          auto imported_project = ProjectFile::fromJson(bundle.at("project"));
          ...
          // TODO: whatever this is doing, it shouldn't use the operation system
          // like this
