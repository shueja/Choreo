...
import { MetersOrFeet, MToFt } from "@choreo/math/UnitConversions";
import { IRobotConfigStore } from "@choreo/stores/RobotConfigStore";

type Props = { rowGap: number; imperial: boolean, config: IRobotConfigStore };

type State = object;

function TheoreticalPanel(props:Props) {
    const config = props.config.snapshot;
    
    const floorSpeed = props.config.wheelMaxVelocity * config.radius;
    const motorFloorLinearForce =
      (4 * props.config.wheelMaxTorque) / config.radius; // N
    ...
    const floorAngularVelocity =
      (props.config.wheelMaxVelocity * config.radius) / driveRadius;
    const imp = props.imperial;
    return (
      <>
        <div
          style={{
        ...
        >
          <InputList noCheckbox rowGap={props.rowGap}>
            <Input
...
export default observer(TheoreticalPanel);
