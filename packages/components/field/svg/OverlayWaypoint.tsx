...
type Props = { waypoint: IWaypointStore; index: number; robotConfig: IRobotConfigStore, pathLength:number };
...
class OverlayWaypoint extends Component<Props, State> {
    ...
    ({
      ...
      <g>
        ...
        <use
          ...
          stroke={strokeColor}
          strokeWidth={strokeWidthPx * 0.0005}
  ...
  getBoxColor() {
    return false//this.props.waypoint.selected
      ? "var(--select-yellow)"
      : "var(--accent-purple)";
  }
  getDragTargetColor(): string {
    const length =this.props.pathLength;
  ...
  render() {
    ...
    const robotConfig = this.props.robotConfig;
