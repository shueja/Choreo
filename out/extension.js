...
const p = "Svelte Extension",
  ...
  r = class r {
    ...
    static createOrShow(e) {
      const t = i.window.activeTextEditor
        ? i.window.activeTextEditor.viewColumn
        : void 0;
      if (r.currentPanel) {
    ...
    dispose() {
      for (
        r.currentPanel = void 0, this._panel.dispose();
...
class m {
  ...
  resolveWebviewView(e) {
    (this._view = e),
      ...
      e.webview.onDidReceiveMessage(async (t) => {
        switch (t.type) {
          case "onInfo": {
            if (!t.value) return;
...
class b {
  ...
  _getHtmlForWebview(e, t) {
    const n = require("path"),
      c = require("fs"),
      o = t.asAbsolutePath(n.join("src", "html", "HTMLView.html"));
...
function y(s) {
  const e = new m(s.extensionUri),
    t = new b(s.extensionUri);
  s.subscriptions.push(i.window.registerWebviewViewProvider("ext-sidebar", e)),
...
function g() {}
export { y as activate, g as deactivate };
