...
import {
  IWaypointStore
} from "@choreo/stores/WaypointStore";
import { PathListStore } from "./PathListStore";
import { RobotConfigStore } from "@choreo/stores/RobotConfigStore";
...
import SwerveSample from "@choreo/document/sample/SwerveSample";
import DifferentialSample from "@choreo/document/sample/DifferentialSample";
  ...
  .views((self) => ({
    ...
    get hoveredWaypointIndex() {
      if (this.isSidebarWaypointHovered) {
        const pt = self.hoveredSidebarItem as IWaypointStore;
        return getParent<IWaypointStore[]>(pt).findIndex(
          (p) => p.uuid == pt.uuid
        );
      } else {
        return undefined;
  ...
  .actions((self) => ({
    ...
    async generatePath(uuid: string) {
      ...
      pathStore.markers.forEach((m) => {
        m.from.setTrajectoryTargetIndex(m.from.getTargetIndex(pathStore.params.waypoints));
      });
...
export const DocumentStoreContext = createContext<IDocumentStore | undefined>(undefined);
