...
import { ViewLayers } from "../../ui/ViewLayers";
import { IStateStore } from "../../../../apps/ext_trajectory/state/State";
import { NavbarLabels } from "../../ui/NavbarData";
...
class FieldOverlayRoot extends Component<Props, State> {
  ...
  render() {
    ...
    const layers = [true, true, true, true, true, true, true];// uiState.layers;
    const constraintSelected = false;//uiState.isConstraintSelected();
    const eventMarkerSelected = false//uiState.isEventMarkerSelected();
    const isNavbarWaypointSelected = false;
    const path = this.props.stateStore.path;
    return (
      <svg
        ref={this.svgRef}
      ...
      >
        <FieldMatrixContext.Provider value={this.state.fieldMatrix}>
          ...
          >
            ...
            {/* Line paths */}
            {layers[ViewLayers.Waypoints] && <FieldPathLines waypoints={path.params.waypoints}></FieldPathLines>}
            {layers[ViewLayers.Trajectory] && (
              <FieldGeneratedLines path={path} gradient={"None"}></FieldGeneratedLines>
            )}
            {layers[ViewLayers.Samples] && layers[ViewLayers.Trajectory] && (
              <FieldSamples trajectory={path.trajectory.fullTrajectory}></FieldSamples>
            )}
            {/* {layers[ViewLayers.Samples] && layers[ViewLayers.Trajectory] && (
              <FieldGeneratedWaypoints></FieldGeneratedWaypoints>
            )} */}
            <FieldEventMarkers path={path}></FieldEventMarkers>
            {layers[ViewLayers.Waypoints] &&
              path.params.waypoints
                .map((point, index) => {
                  ...
                  if (
                    ...
                    return [
                      <OverlayWaypoint
                        waypoint={point}
                        index={index}
                        key={point.uuid} robotConfig={this.props.stateStore.config} pathLength={path.params.waypoints.length}                      ></OverlayWaypoint>,
                      false// point.selected
                    ];
                  }
                })
            ...
            {eventMarkerSelected && (
              <FieldEventMarkerAddLayer path={path}></FieldEventMarkerAddLayer>
            )}
            {/* {false && (
            ...
            {layers[ViewLayers.Trajectory] && (
              <InterpolatedRobot
                timestamp={0.1} robotConfig={this.props.stateStore.config} trajectory={path.trajectory.fullTrajectory}              ></InterpolatedRobot>
            )}
          </g>
  ...
  createWaypoint(e: MouseEvent, waypointType: number): void {
    ...
    const coords = this.screenSpaceToFieldSpace(this.svgRef?.current, {
      ...
      const newPoint = this.props.stateStore.path.params.addWaypoint(initial);
